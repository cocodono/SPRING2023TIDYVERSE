...
author: "Keith Colella, John Cruz"
date: "`r Sys.Date()`"
...
The data for this example will be using [MTA subway station locations](https://data.ny.gov/widgets/i9wp-a4ja), which provides latitude and longitude for every subway exit and its subway line. In addition, I will be using [data](https://data.cityofnewyork.us/Health/Hospitals/833h-xwsx) on NYC's [public acute care hospitals](https://portal.311.nyc.gov/article/?kanumber=KA-02793) throughout the NYC region. This dataset also provides latitude and longitude information. 
...
```{r import-mta, message=FALSE, warning=FALSE}
library(janitor)
...
```{r fuzzy-geo-join, message=FALSE}
station_hhc <-
  hhc_locations |> 
