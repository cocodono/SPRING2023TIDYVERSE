...
```
-------------------------------------------------------------------------------
# Susanna Tidyverse Extend {.tabset}
...
We will need the following portion of the original code 
...
```{r}
long_table <- bob_ross %>% 
  pivot_longer(cols = !c('title','season','episode',"episode_num","title"), names_to = "color", values_to = "count" )
...
season <- season %>%
  group_by(season) %>%
  filter(!color%in% c('tree', 'trees')) %>% # If we did not filter out the "tree" or "trees", the top element for all season will be "tree" or "trees"
  arrange(desc(count)) %>% 
  slice(1)
