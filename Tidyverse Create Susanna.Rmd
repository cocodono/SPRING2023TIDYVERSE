...
# Joe Garcia's Extend Assignment*****
I decided to extend Susanna Wong's College Grads dataset, because used the same dataset. Here I just changed Women to Men! Good job Susanna!
...
```{r}
# Joe Garcia Extending Susanna's code, Here I added the men as well!
college_major <- raw_data %>%
  group_by(Major_category) %>% 
  summarise(Total = sum(Total,na.rm=TRUE),
            Men = sum(Men,na.rm=TRUE),
...
ggplot(data = college_major, aes(y = reorder(Major_category, -ShareMen), x = ShareMen)) + geom_bar(stat="identity")
...
https://github.com/suswong/SPRING2023TIDYVERSE/blob/main/TidyVerse%20Extend%20on%20JLok's%20Create%20Assignment.Rmd
https://rpubs.com/sussing/1035929
